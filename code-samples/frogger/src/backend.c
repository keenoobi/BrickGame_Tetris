...
void add_proggress(board_t *map) {
  ...
  for (int progress_cnt = BOARD_WIDTH / 5; progress_cnt > 0; progress_cnt--)
...
bool check_level_compl(board_t *map) {
  bool rc = TRUE;
  for (int i = 0; i < BOARD_WIDTH && rc; i++)
...
void fill_finish(char *finish_line) {
  for (int i = 0; i < BOARD_WIDTH; i++) finish_line[i] = ' ';

  finish_line[BOARD_WIDTH] = '\0';
}
